...
# Access (requires /etc/hosts configuration)
...
#
# /etc/hosts entries:
# 127.0.0.1    localhost.startupwebapp.com
# 127.0.0.1    localapi.startupwebapp.com
```
...
#### 1. **Stripe Integration Upgrade** (CRITICAL - In Progress)
   ...
   **Status**: Phase 5.16 multi-session project

   **Problem:**
   - Current: Stripe Checkout v2 (deprecated, non-functional)
   - Library: stripe==5.5.0 (severely outdated)
   ...
   **Solution:**
   ...
   - Upgrade stripe library to 10.x+
   - 10-session plan documented in `docs/technical-notes/2025-12-11-stripe-upgrade-plan.md`

   **Current Session Plan:**
   - Session 1: ✅ Planning & assessment complete
   - Session 2: Library upgrade (stripe 5.5.0 → 10.x)
   - Session 3: Create checkout session endpoint
...
#### 2. **Email Address Updates** (Ready to Deploy - After Stripe)
   ...
   **Status**: Code complete in `bugfix/email-addresses` branch, NOT yet deployed
