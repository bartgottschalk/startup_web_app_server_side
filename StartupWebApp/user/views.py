...
def logged_in(request):
    ...
    if request.user.is_authenticated:
        ...
        response = JsonResponse(
            {
                'logged_in': True,
                'log_client_events': ClientEventConfiguration.objects.get(id=1).log_client_events,
                'client_event_id': request.user.id,
                'member_initials': member_initials,
                'first_name': request.user.first_name,
                'last_name': request.user.last_name,
                'email_address': request.user.email,
                'cart_item_count': cart_item_count,
                'user-api-version': user_api_version,
            },
            safe=False,
        )
    else:
        ...
        if (
            request.get_signed_cookie(
                key='anonymousclientevent', default=False, salt='clienteventanonymousclienteventoccurrence'
            )
            == False  # noqa: E712
        ):
    ...
    if user is not None:
        ...
        if member_cart is None:
            ...
            if anonymous_cart is not None:
                # logger.debug('Anonymous cart found during registration')
            ...
            if anonymous_cart is not None:
                ...
                # skus. put any skus from the anonymous cart into the member cart if
                # they're not already there
                for anonymous_cart_sku in Cartsku.objects.filter(cart=anonymous_cart):
                    sku_exists_in_member_cart = Cartsku.objects.filter(
                        cart=member_cart, sku=anonymous_cart_sku.sku
                    ).exists()
                ...
                if not member_cart_shipping_method_exists:
                    anonymous_cart_shipping_method_exists = Cartshippingmethod.objects.filter(
                        cart=anonymous_cart
                    ).exists()
                    if anonymous_cart_shipping_method_exists:
                        Cartshippingmethod.objects.create(
                            cart=member_cart,
                            shippingmethod=Cartshippingmethod.objects.get(cart=anonymous_cart).shippingmethod,
                        )
...
def create_account(request):
    ...
    if (
        ...
        most_recent_terms_of_use_version_obj = Termsofuse.objects.get(
            version=most_recent_terms_of_use_version['version__max']
        )
        now = timezone.now()
        Membertermsofuseversionagreed.objects.create(
            member=member, termsofuseversion=most_recent_terms_of_use_version_obj, agreed_date_time=now
        )
        ...
        if user is not None:
            ...
            if Prospect.objects.filter(email=email_address).exists():
                ...
                if prospect.swa_comment is None:
                    ...
                    swa_comment_str = (
                        prospect.swa_comment + '. Prospect converted by creating user id ' + str(user.id) + '.'
                    )
            ...
            welcome_email_content += (
                ...
                + '. We\'re excited that you\'ve signed up and we hope you enjoy using our product as a member!'
            )
            welcome_email_content += '\r\n\r\n'
            welcome_email_content += (
                'A quick reminder that you registered with the username "' + request.user.username + '".'
            )
            welcome_email_content += '\r\n\r\n'
            welcome_email_content += (
                'While we have your attention, we\'d love it if you took a few seconds to '
                'verify your email address. If it really was you who registered for an account at '
                + settings.ENVIRONMENT_DOMAIN
                + ', please go to the following URL to confirm that you are authorized to use this '
                'email address:'
            )
            welcome_email_content += '\r\n\r\n'
            ...
            welcome_email_content += '\r\n\r\n'
            welcome_email_content += (
                'Your request to verify this email address will not be processed unless you '
                'confirm the address using this URL. This link expires 24 hours after you '
                'registered. However, you can always verify your email address at a future date '
                'using the options on your account page.'
            )
            welcome_email_content += '\r\n\r\n'
            welcome_email_content += (
            ...
            email = EmailMessage(
                ...
                from_email='bart+startupwebapp@mosaicmeshai.com',
                to=[request.user.email],
                reply_to=['bart+startupwebapp@mosaicmeshai.com'],
            )
            try:
            ...
            # Return an 'invalid login' error message.
            logger.error('Registration failed - user created but login failed')
            return JsonResponse(
                {'create_account': 'created_but_login_failed', 'user-api-version': user_api_version}, safe=False
            )
    else:
        error_dict = {
            ...
            "email-address": email_address_valid,
            "password": password_valid,
        }
        return JsonResponse(
            {'create_account': 'false', 'errors': error_dict, 'user-api-version': user_api_version}, safe=False
        )
...
def verify_email_address_response(request):
    ...
    if request.user.is_anonymous:
        logger.warning('AnonymousUser found when authenticated user expected')
        return JsonResponse(
            {'verify_email_address_response': 'user_not_authenticated', 'user-api-version': user_api_version},
            safe=False,
        )

    email_verification_code = request.POST['email_verification_code']
    try:
        unsigned_string = email_verification_signer.unsign(
            email_verification_code, max_age=86400
        )  # 86400 seconds is one day
        if request.user.member.email_verification_string == unsigned_string:
            ...
            request.user.member.save()
            return JsonResponse(
                {'verify_email_address_response': 'success', 'user-api-version': user_api_version}, safe=False
            )
        else:
            return JsonResponse(
                {'verify_email_address_response': 'code-doesnt-match', 'user-api-version': user_api_version}, safe=False
            )
    except BadSignature:
        return JsonResponse(
            {'verify_email_address_response': 'signature-invalid', 'user-api-version': user_api_version}, safe=False
        )
    except SignatureExpired:
        return JsonResponse(
            {'verify_email_address_response': 'signature-expired', 'user-api-version': user_api_version}, safe=False
        )
...
def reset_password(request):
    ...
    try:
        ...
        if user.email.lower() == email_address.lower():
            ...
            reset_password_content += (
                'We have received a request to send you a password reset link the username "'
                + username
                + '"" at '
                + settings.ENVIRONMENT_DOMAIN
                + '. If you requested this password reset, please go to the following URL to complete '
                'the password reset process:'
            )
            reset_password_content += '\r\n\r\n'
            reset_password_content += (
                settings.ENVIRONMENT_DOMAIN + '/set-new-password?password_reset_code=' + signed_string
            )
            reset_password_content += '\r\n\r\n'
            reset_password_content += (
                'Your request to reset your password will not be processed unless you complete the '
                'process using this URL. This link expires 1 hours after your original password '
                'reset request.'
            )
            reset_password_content += '\r\n\r\n'
            reset_password_content += (
            ...
            try:
                ...
                logger.exception(f'SMTPDataError sending forgot username email to {email_address}')
        else:
            logger.warning(f'Email signature mismatch: {user.email} != {email_address}')
    except ObjectDoesNotExist as e:
        logger.warning(f'User not found in forgot_username for email {email_address}: {e}')
...
def set_new_password(request):
    ...
    try:
        ...
        try:
            unsigned_string = reset_password_signer.unsign(
                request.POST['password_reset_code'], max_age=86400
            )  # 86400 seconds is one day
            if user.member.reset_password_string == unsigned_string:
                ...
                if password_valid == True:  # noqa: E712
                    ...
                    set_new_password_confirmation_content += (
                        'Your account password has been changed for the username "'
                        + username
                        + '" at '
                        + settings.ENVIRONMENT_DOMAIN
                    ...
                    try:
                        ...
                        logger.exception('SMTPDataError sending password reset confirmation email')

                    return JsonResponse(
                        {'set_new_password': 'success', 'user-api-version': user_api_version}, safe=False
                    )
                else:
                    logger.warning('VALIDATION ERRORS - RETURN ERRORS in set_new_password')
                    error_dict = {"new-password": password_valid}
                    return JsonResponse(
                        {
                            'set_new_password': 'password-error',
                            'errors': error_dict,
                            'user-api-version': user_api_version,
                        },
                        safe=False,
                ...
                return JsonResponse(
                    {'set_new_password': 'code-doesnt-match', 'user-api-version': user_api_version}, safe=False
                )
        except BadSignature:
            return JsonResponse(
                {'set_new_password': 'signature-invalid', 'user-api-version': user_api_version}, safe=False
            )
        except SignatureExpired:
            return JsonResponse(
                {'set_new_password': 'signature-expired', 'user-api-version': user_api_version}, safe=False
            )
    except ObjectDoesNotExist as e:
        logger.warning(f'User not found in set_new_password for username {username}: {e}')
        return JsonResponse(
            {
                'set_new_password': 'username-not-found',
...
def forgot_username(request):
    ...
    try:
        ...
        for user in users:
            ...
            forgot_username_content += (
                'We received a request to resend your username at ' + settings.ENVIRONMENT_DOMAIN + '.'
            )
            forgot_username_content += '\r\n\r\n'
            forgot_username_content += (
                'The username for the account associated with '
                + email_address
                + ' is "'
                + user.username
                + '". Please go to the following URL to login:'
            )
            forgot_username_content += '\r\n\r\n'
            ...
            try:
                ...
                logger.exception('SMTPDataError sending forgot username email')
    except ObjectDoesNotExist as e:
        logger.warning(f'User not found in forgot_username for email {email_address}: {e}')
...
def update_my_information(request):
    ...
    if firstname_valid == True and lastname_valid == True and email_address_valid == True:  # noqa: E712
        logger.info('VALIDATION PASSED - UPDATE USER INFO')
        ...
        if old_email_address != request.user.email:
            ...
            edit_my_information_email_content += (
                ...
                + settings.ENVIRONMENT_DOMAIN
                + ' account has been modified. This email has been sent to both the previous and new '
            ...
            edit_my_information_email_content += '\r\n\r\n'
            edit_my_information_email_content += (
                'While we have your attention, we\'d love it if you took a few seconds to verify your '
                'new email address. Please go to the following URL to confirm that you are authorized '
                'to use this new email address:'
            )
            edit_my_information_email_content += '\r\n\r\n'
            edit_my_information_email_content += (
                settings.ENVIRONMENT_DOMAIN + '/account/?email_verification_code=' + signed_string
            )
            edit_my_information_email_content += '\r\n\r\n'
            edit_my_information_email_content += (
                'Your request to verify this new email address will not be processed unless you '
                'confirm the address using this URL. This link expires 24 hours after you changed email '
                'addresses. However, you can always verify your new email address at a future date using '
                'the options on your account page.'
            )
            edit_my_information_email_content += '\r\n\r\n'
            edit_my_information_email_content += (
            ...
            try:
                ...
                logger.exception('SMTPDataError sending email verification during info update')

            return JsonResponse({'update_my_information': 'success', 'user-api-version': user_api_version}, safe=False)
        else:
            logger.debug('Email address did not change, skipping verification email')
            return JsonResponse({'update_my_information': 'success', 'user-api-version': user_api_version}, safe=False)
    else:
        logger.warning('VALIDATION ERRORS - RETURN ERRORS')
        error_dict = {"firstname": firstname_valid, "lastname": lastname_valid, "email-address": email_address_valid}
        return JsonResponse(
            {'update_my_information': 'errors', 'errors': error_dict, 'user-api-version': user_api_version}, safe=False
        )
...
def update_communication_preferences(request):
    ...
    if request.method == 'POST' and 'newsletter' in request.POST and 'email_unsubscribe' in request.POST:
        ...
        if newsletter == 'true' and email_unsubscribe == 'true':
            ...
            return JsonResponse(
                {
                    'update_communication_preferences': 'errors',
                    'errors': error_dict,
                    'user-api-version': user_api_version,
                },
                safe=False,
            ...
            # reset email unsubscribe string if the value of email_unsubscribe is
            # changing from what it was
            logger.debug(
                f'User email_unsubscribed status: {
                    request.user.member.email_unsubscribed}'
            )
            ...
            if no_longer_want_to_receive_val or never_signed_up_val or inappropriate_val or spam_val or other_val:
                ...
                Emailunsubscribereasons.objects.create(
                    ...
                    no_longer_want_to_receive=no_longer_want_to_receive_val,
                    never_signed_up=never_signed_up_val,
                    inappropriate=inappropriate_val,
                    spam=spam_val,
                    other=other_val,
        ...
        return JsonResponse(
            {'update_communication_preferences': 'errors', 'errors': error_dict, 'user-api-version': user_api_version},
            safe=False,
        )

    return JsonResponse(
        {'update_communication_preferences': 'success', 'user-api-version': user_api_version}, safe=False
    )
...
def change_my_password(request):
    ...
    if request.user.is_anonymous:
        logger.warning('AnonymousUser found when authenticated user expected')
        return JsonResponse(
            {'change_my_password': 'user_not_authenticated', 'user-api-version': user_api_version}, safe=False
        )
    ...
    if not request.user.check_password(current_password):
        ...
        return JsonResponse(
            {'change_my_password': 'errors', 'errors': error_dict, 'user-api-version': user_api_version}, safe=False
        )
    elif request.user.check_password(password):
        error_dict = {
            "password": [
                {
                    'type': 'new_password_same_as_current_password',
        ...
        if password_valid == True and request.user.check_password(current_password) == True:  # noqa: E712
            ...
            if user is not None:
                ...
                try:
                    email.send(fail_silently=False)
                    return JsonResponse(
                        {'change_my_password': 'success', 'user-api-version': user_api_version}, safe=False
                    )
                except SMTPDataError:
                    logger.exception('SMTPDataError sending password change confirmation email')
                ...
                logger.error('Registration failed - user created but login failed')
                return JsonResponse(
                    {'change_my_password': 'changed_password_but_login_failed', 'user-api-version': user_api_version},
                    safe=False,
                )
        else:
            logger.warning('VALIDATION ERRORS - RETURN ERRORS')
    ...
    if request.method == 'GET' and ('token' in request.GET or 'pr_token' in request.GET):
        ...
        try:
            if token is not None:
                logger.debug('Looking up member for email unsubscribe')
                ...
                logger.debug('Looking up prospect for email unsubscribe')
            ...
            if not member_or_prospect.email_unsubscribed:
                unsigned_value = email_unsubscribe_signer.unsign(
                    member_or_prospect.email_unsubscribe_string + ':' + token_val
                )
                if unsigned_value == member_or_prospect.email_unsubscribe_string:
                    ...
                    response = JsonResponse(
                        {
                            'email_unsubscribe_lookup': 'success',
                    ...
                    logger.warning("Email unsubscribe lookup: token-invalid")
                    error_dict = {"error": 'token-invalid'}
                    response = JsonResponse(
                        {
                            'email_unsubscribe_lookup': 'error',
                            'errors': error_dict,
                            'user-api-version': user_api_version,
                        },
                        safe=False,
                ...
                logger.info("Email address already unsubscribed")
                error_dict = {"error": 'email-address-already-unsubscribed'}
                response = JsonResponse(
                    {'email_unsubscribe_lookup': 'error', 'errors': error_dict, 'user-api-version': user_api_version},
                    safe=False,
                )
        except BadSignature as e:
            logger.warning(f"Email unsubscribe lookup: token-altered - {e}")
            ...
            logger.warning(f"Email unsubscribe lookup: member not found - {e}")
        ...
        try:
            ...
            if not member_or_prospect.email_unsubscribed:
                ...
                if unsigned_value == member_or_prospect.email_unsubscribe_string:
                    ...
                    response = JsonResponse(
                        {
                            'email_unsubscribe_confirm': 'success',
                            'email_address': masked_email_address,
                            'token': signed_string.rsplit(':', 1)[1],
                            'user-api-version': user_api_version,
                        },
                        safe=False,
                    )
                    ...
                    response = JsonResponse(
                        {
                            'email_unsubscribe_confirm': 'error',
                            'errors': error_dict,
                            'user-api-version': user_api_version,
                        },
                        safe=False,
                ...
                error_dict = {"error": 'email-address-already-unsubscribed'}
                response = JsonResponse(
                    {'email_unsubscribe_confirm': 'error', 'errors': error_dict, 'user-api-version': user_api_version},
                    safe=False,
                )
        except BadSignature as e:
            logger.warning(f"Email unsubscribe confirm: token-altered - {e}")
            error_dict = {"error": 'token-altered'}
            response = JsonResponse(
                {'email_unsubscribe_confirm': 'error', 'errors': error_dict, 'user-api-version': user_api_version},
                safe=False,
            ...
            logger.warning(f"Email unsubscribe confirm: member not found - {e}")
            error_dict = {"error": 'member-not-found'}
            response = JsonResponse(
                {'email_unsubscribe_confirm': 'error', 'errors': error_dict, 'user-api-version': user_api_version},
                safe=False,
...
def email_unsubscribe_why(request):
    ...
    if request.method == 'POST' and ('token' in request.POST or 'pr_token' in request.POST):
        ...
        try:
            ...
            if unsigned_value == member_or_prospect.email_unsubscribe_string:
                logger.debug('Reached email unsubscribe confirmation')
                ...
                response = JsonResponse(
                    {'email_unsubscribe_why': 'success', 'user-api-version': user_api_version}, safe=False
                )
            else:
                logger.warning("Email unsubscribe lookup: token-invalid")
                error_dict = {"error": 'token-invalid'}
                response = JsonResponse(
                    {'email_unsubscribe_why': 'error', 'errors': error_dict, 'user-api-version': user_api_version},
                    safe=False,
                )
        except BadSignature as e:
            logger.warning(f"Email unsubscribe why: token-altered - {e}")
            error_dict = {"error": 'token-altered'}
            response = JsonResponse(
                {'email_unsubscribe_why': 'error', 'errors': error_dict, 'user-api-version': user_api_version},
                safe=False,
            ...
            logger.warning(f"Email unsubscribe why: member not found - {e}")
            error_dict = {"error": 'member-not-found'}
            response = JsonResponse(
                {'email_unsubscribe_why': 'error', 'errors': error_dict, 'user-api-version': user_api_version},
                safe=False,
        ...
        error_dict = {"error": 'token-required'}
        response = JsonResponse(
            {'email_unsubscribe_why': 'error', 'errors': error_dict, 'user-api-version': user_api_version}, safe=False
        )
    return response
...
def terms_of_use_agree_check(request):
    # raise ValueError('A very specific bad thing happened.')
    most_recent_terms_of_use_version = Termsofuse.objects.all().aggregate(Max('version'))
    membertermsofuseversionagreed_exists = Membertermsofuseversionagreed.objects.filter(
        member=request.user.member, termsofuseversion=most_recent_terms_of_use_version['version__max']
    ).exists()
    if membertermsofuseversionagreed_exists:
        ...
        response = JsonResponse(
            {
                'terms_of_use_agree_check': False,
                'version': most_recent_terms_of_use_version_obj.version,
                'version_note': most_recent_terms_of_use_version_obj.version_note,
                'user-api-version': user_api_version,
            },
            safe=False,
        )
    ...
    if version is not None:
        try:
            ...
            if request_version_obj.id == most_recent_terms_of_use_version_obj.id:
                ...
                try:
                    Membertermsofuseversionagreed.objects.create(
                        member=request.user.member, termsofuseversion=request_version_obj, agreed_date_time=now
                    )
                    response = JsonResponse(
                        {'terms_of_use_agree': 'success', 'version': version, 'user-api-version': user_api_version},
                        safe=False,
                    )
                except IntegrityError as e:
                    logger.warning(f'Terms of use already agreed for version {version}: {e}')
            ...
            logger.warning(f'Terms of use version not found: {version} - {e}')
...
def put_chat_message(request):
    ...
    if name_valid == True and email_address_valid == True and message_valid == True:  # noqa: E712
        ...
        if request.user.is_authenticated:
            ...
            Chatmessage.objects.create(
                member=User.objects.get(email=email_address).member,
                prospect=None,
                name=name,
                email_address=email_address,
                message=message,
            ...
            if Prospect.objects.filter(email=email_address).exists():
                ...
                prospect = Prospect.objects.create(
                    ...
                    swa_comment='Captured from chat message submission',
                    pr_cd=pr_cd,
                    created_date_time=now,
                )
                Chatmessage.objects.create(
        ...
        try:
        ...
        except (SMTPDataError, ConnectionRefusedError, OSError, ValueError):
            logger.exception('Failed to send chat message email')
            ...
            if settings.DEBUG:
                ...
                return JsonResponse(
                    {'put_chat_message': 'email_failed', 'errors': error_dict, 'user-api-version': user_api_version},
                    safe=False,
                )
        ...
        return JsonResponse(
            {'put_chat_message': 'validation_error', 'errors': error_dict, 'user-api-version': user_api_version},
            safe=False,
        )
...
def pythonabot_notify_me(request):
    ...
    if email_address_valid == True and how_excited_valid == True:  # noqa: E712
        if Prospect.objects.filter(email=email_address).exists():
            prospect_errors = []
            duplicate_prospect_error = {
                'type': 'duplicate',
                'description': 'I already know about this email address. Please enter a different email address.',
            }
            prospect_errors.append(duplicate_prospect_error)
            error_dict = {"email_address": prospect_errors, "how_excited": how_excited_valid}
            return JsonResponse(
                {
                    'pythonabot_notify_me': 'duplicate_prospect',
                    'errors': error_dict,
                    'user-api-version': user_api_version,
                },
                safe=False,
            ...
            Prospect.objects.create(
                ...
                email_unsubscribe_string=email_unsubscribe_string,
                email_unsubscribe_string_signed=email_unsubscribe_string_signed,
                prospect_comment=str(how_excited) + ' on a scale of 1-5 for how excited they are',
                swa_comment='This prospect would like to be notified when PythonABot is ready to be purchased.',
                pr_cd=pr_cd,
                created_date_time=now,
            )
        ...
        return JsonResponse(
            {'pythonabot_notify_me': 'validation_error', 'errors': error_dict, 'user-api-version': user_api_version},
            safe=False,
        )
