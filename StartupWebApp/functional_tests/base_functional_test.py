...
from user.models import Termsofuse, Emailtype, Emailstatus, Adtype, Adstatus
...
class BaseFunctionalTest(LiveServerTestCase):
    ...
    host = '0.0.0.0'  # Bind to all interfaces (accessible via Docker network)
    # Hardcoded port for predictable LiveServerTestCase API access
    port = 60767
    yesterday = start_date_time = timezone.now() - timedelta(days=1)
    tomorrow = start_date_time = timezone.now() + timedelta(days=1)
    # Use localliveservertestcase.startupwebapp.com for cookie sharing
    # Two environments supported:
    # 1. CI (CI_ENV=true): Frontend on port 8080, backend on port 60767
    # 2. Docker (DOCKER_ENV=true): Frontend on port 80, backend on port 60767
    if os.environ.get('CI_ENV'):
    ...
    def setUp(self):
        ...
        Status.objects.create(
            identifier='accepted',
            title='Accepted',
            description='The order has been accepted and is being processed.'
        )
        Status.objects.create(
        ...
        Shippingmethod.objects.create(
            carrier='USPS Priority Mail 2-Day',
            shipping_cost='13.65',
            tracking_code_base_url=(
                'https://tools.usps.com/go/TrackConfirmAction.action?'
                'tRef=fullpage&tLc=1&text28777=&tLabels='
            ),
            active=True,
            identifier='USPSPriorityMail2Day'
        )
        Shippingmethod.objects.create(
        ...
        Shippingmethod.objects.create(
            ...
            tracking_code_base_url=(
                ...
                'action=track&cntry_code=us&trackingnumber='
            ),
            active=True,
        ...
        Shippingmethod.objects.create(
            ...
            tracking_code_base_url=(
                ...
                '&track.x=Track&trackNums='
            ),
            active=True,
            identifier='UPSGround'
        )
        Shippingmethod.objects.create(
            carrier='None',
            shipping_cost='0.00',
